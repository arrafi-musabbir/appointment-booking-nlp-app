...
credentials = Credentials.from_service_account_file('onguard-394009-bbf0f65d62ae.json', scopes=scopes)
...
# open a google sheet
gs = gc.open_by_key('1syaoV1PY9WdFoBMaxlwpsGSrBWNzhduOlmU0SA4Jx7o')# select a work sheet from its name
worksheet1 = gs.worksheet('responses')
...
def process_string():
    data = request.args.get('sms')
    try:
        if len(data) == 0: 
            print("None")
            return jsonify('no data given')
        else:
            processed_data = classifyText(data)
            df = pd.DataFrame({'a':[str(datetime.now())], 'b':[data], 'c':[processed_data]})
            df_values = df.values.tolist()
            gs.values_append('responses', {'valueInputOption': 'RAW'}, {'values': df_values})
